	...
	test -f /etc/picoinfluxfritzboxes  && test -f ~/.picoinflux.conf &&	(
	 cat /etc/picoinfluxfritzboxes | while read myline;do
		...
		#echo $hostname $hostip
			...
			(
			#echo CONN #/tmp/.picofritz/conn_st.xml
			conn_st=$(echo "" | base64 -d)
			echo "$conn_st"|		curl -s --connect-timeout 5 "http://"$hostip":"49000"/igdupnp/control/WANIPConn1" -H "Content-Type: text/xml; charset="utf-8"" -H "SoapAction:urn:schemas-upnp-org:service:WANIPConnection:1#GetStatusInfo" -d "@-" 2>&1 |grep New
			#echo EXTIP
			echo "$conn_st"|		curl -s --connect-timeout 5 "http://"$hostip":"49000"/igdupnp/control/WANIPConn1" -H "Content-Type: text/xml; charset="utf-8"" -H "SoapAction:urn:schemas-upnp-org:service:WANIPConnection:1#GetExternalIPAddress" -d "@-"  2>&1 |grep New
			#echo LINK
			link_st=$(echo "" |base64 -d )
			# > /tmp/.picofritz/link_st.xml |grep New
			echo "$link_st"|curl -s --connect-timeout 5 "http://"$hostip":"49000"/igdupnp/control/WANCommonIFC1" -H "Content-Type: text/xml; charset="utf-8"" -H "SoapAction:urn:schemas-upnp-org:service:WANCommonInterfaceConfig:1#GetCommonLinkProperties" -d "@-" 2>&1 |grep New
			#echo TRAFFIC
			echo "$link_st"|curl -s --connect-timeout 5 "http://"$hostip":"49000"/igdupnp/control/WANCommonIFC1" -H "Content-Type: text/xml; charset="utf-8"" -H "SoapAction:urn:schemas-upnp-org:service:WANCommonInterfaceConfig:1#GetAddonInfos" -d "@-" 2>&1 |grep -e NewX_AVM_DE_TotalBytes -e NewByteSendRate -e NewByteReceiveRate 
